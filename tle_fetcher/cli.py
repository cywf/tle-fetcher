...
from .connectors import NotionSync
...
def build_sync_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Synchronize cached data with external services.")
    parser.add_argument("command", choices=["sync"], help="Subcommand to execute.")
    parser.add_argument("--target", choices=["notion"], default="notion",
                        help="Connector target to synchronize.")
    parser.add_argument("--mode", choices=["pull", "push", "both"], default="pull",
                        help="Whether to pull from Notion, push local data, or both.")
    parser.add_argument("--output", default="data/notion_dump.json",
                        help="File to save Notion results when pulling.")
    parser.add_argument("--input", default="data/notion_dump.json",
                        help="File containing records to push when mode includes push.")
    parser.add_argument("--limit", type=int, help="Maximum number of records to push.")
    parser.add_argument("--page-size", type=int, default=50, help="Page size when pulling from Notion.")
    return parser
...
def run_sync(ns: argparse.Namespace) -> int:
    logging.basicConfig(level=logging.INFO)
    config = load_config()

    if ns.target != "notion":
        print(f"Unsupported sync target: {ns.target}", file=sys.stderr)
        return 2

    connector = NotionSync(config)
    exit_code = 0

    if ns.mode in {"pull", "both"}:
        output_path = Path(ns.output)
        if not output_path.is_absolute():
            output_path = config.base_dir / output_path
        result = connector.pull(output_path, page_size=ns.page_size)
        if result.status == "error":
            print(f"Notion pull failed: {result.reason}", file=sys.stderr)
            exit_code = 2
        elif result.status == "skipped":
            logging.info("Notion pull skipped (%s).", result.reason)

    if ns.mode in {"push", "both"}:
        input_path = Path(ns.input)
        if not input_path.is_absolute():
            input_path = config.base_dir / input_path
        result = connector.push(input_path, limit=ns.limit)
        if result.status == "error":
            print(f"Notion push failed: {result.reason}", file=sys.stderr)
            exit_code = 2
        elif result.status == "skipped":
            logging.info("Notion push skipped (%s).", result.reason)

    return exit_code


def run_cli(ns: argparse.Namespace) -> int:
    if getattr(ns, "command", "fetch") == "sync":
        return run_sync(ns)

    if ns.ids_file:
        ids_path = Path(ns.ids_file)
